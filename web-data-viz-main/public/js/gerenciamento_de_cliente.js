...
// quantidade de analistas por zona (mock)
const ANALISTAS_POR_ZONA = {
    'SP-01': 3,
    'SP-02': 2,
    'MG-01': 2,
    'default': 3
...
async function dashprocesso() {
    try {
        const resp = await fetch('../dados/tickets.json');
        const txt  = await resp.text();

        console.log('Resposta de /dados/tickets.json:', resp.status, txt);
...
function obterQtdAnalistas(zona) {
    if (zona === 'all') {
        const zonas = [...new Set(
            dadosTickets
                .map(t => t.zona || t.identificador?.zona_nome)
                .filter(Boolean)
        )];

        return zonas.reduce((acc, z) => {
            const qtd = ANALISTAS_POR_ZONA[z] ?? ANALISTAS_POR_ZONA.default ?? 0;
            return acc + qtd;
        }, 0);
    }

    return ANALISTAS_POR_ZONA[zona] ?? ANALISTAS_POR_ZONA.default ?? 0;
}

function atualizarKpiEquipe(zonaAtual, ticketsZona) {
    const elQtdAnalistas = document.getElementById('teamAnalystsValue');
    ...
    if (!elQtdAnalistas || !elMttr || !elBadge || !elPctSla) return;

    const numAnalistas   = obterQtdAnalistas(zonaAtual);
    const mttrEquipe     = calcularMTTRGeralHoras(ticketsZona);
    const { pctDentro }  = calcularSLADaEquipe(ticketsZona, SLA_MTTR_HORAS);

    elQtdAnalistas.textContent = numAnalistas;
...
function atualizarTituloDashboard() {
    ...
    if (select.value === 'all') {
        titleElement.textContent = 'Dashboard - Performance de Resoluções dos Tickets das Zonas (Últimos 7 dias)';
    } else {
        titleElement.textContent = `Dashboard - Performance de Resoluções dos Tickets das Zonas - ${selectedZone} (Últimos 7 dias)`;
    }
}
